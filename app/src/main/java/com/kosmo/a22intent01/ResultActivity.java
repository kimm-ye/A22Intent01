...
public class ResultActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        /*
        Intent를 통해 전달되는 부가데이터는 Bundle객체를 통해 액비티비간에
        전송된다. 액티비티가 실행될때 제일 처음 호출되는 생멍주기 메서드가
        onCreate()인데 이때 파라미터 형태로 전송받게 된다.
         */
        //메서드를 통해 Intent 객체를 얻어온다.
        Intent intent = getIntent();
        //번들객체를 얻어온다.
        Bundle bundle = intent.getExtras();
        //투표수를 가져온다. 방법1. 번들로도 가져올수 있고
        int[] votes = bundle.getIntArray("votes");
        //영화제목을 가져온다. 방법2. 인텐트로도 가져올 수 있다.
